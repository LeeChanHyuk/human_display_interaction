...
def main_user_classification(face_center_coordinate, head_poses = None, use_head_pose = True, use_center_people = True):
    if head_poses is None:
        ...
        for index, human_info in enumerate(head_poses):
            ...
            if use_center_people:
                man_score[index] -= abs(face_center_coordinate[index][0] - 320)
            if use_head_pose and (abs(head_poses[index][1]) > 50 or abs(head_poses[index][0] > 30)): # if yaw value of the man is over than 30 or -30, then we think that the man is not looking the display.
                man_score[index] = 0
        max_val = max(man_score)
        main_user_index = man_score.index(max_val)
        return main_user_index
